...
OpenCL, have exposed these systems to the HPC community through
so-called GPGPU, General-purpose computing on graphics processing
units. These libraries exposes low level APIs and require the
applications to be designed in languages like C/C++.
...
efforts. It describes PTX.jl a library to make the GPU programmable
from a high level language. The language chosen is the Julia
programming language that targets technical computing, built on the
LLVM infrastructure.
\end{abstract}
